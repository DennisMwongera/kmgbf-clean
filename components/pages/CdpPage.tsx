...
// import { gapItems, getDimScores, TIMELINE_CANONICAL, toCanonical, fromCanonical } from '@/lib/utils'
// import { SectionActions } from '@/components/ui'
// import { Plus, Trash2 } from 'lucide-react'

// // ─── Single action row inputs ──────────────────────────────────
...
//     institution:   initialRow.institution,
//     timeline:      initialRow.timeline,
//     budget:        initialRow.budget,
//     indicator:     initialRow.indicator,
//     collaboration: initialRow.collaboration,
//   })
...
//     <div>
//       <div className="text-[9px] font-bold uppercase tracking-wide text-forest-400 mb-1">{ph}</div>
...
//   return (
//     <div className="grid gap-2.5 p-3 rounded-xl bg-sand-50 border border-sand-300 mb-2 group relative"
//       style={{ gridTemplateColumns:'2.5fr 1.5fr 1fr 1fr 1.5fr 2fr 28px' }}>
//       {ta('action',        t.common.action)}
//       {ta('institution',   t.common.institution)}
//       <div>
//         <div className="text-[9px] font-bold uppercase tracking-wide text-forest-400 mb-1">
...
// // ─── Single gap subsection within a dimension ─────────────────
// function GapSubsection({ gap, cdpRows, allIndices, onUpdate, onRemove, onAddAction, dim, t, readOnly }: {
//   gap: string; dim: string
//   cdpRows: CdpRow[]; allIndices: number[]
//   onUpdate: (idx: number, field: string, val: string) => void
//   onRemove: (idx: number) => void
//   onAddAction: (dim: string, gap: string) => void
//   t: ReturnType<typeof getT>
//   readOnly?: boolean
// }) {
//   // Pair rows that match this gap with their global indices
...
//   return (
//     <div className="mb-4 rounded-xl border border-sand-200 overflow-hidden">
//       {/* Gap header row */}
...
//             {paired.length} action{paired.length!==1?'s':''}
//           </span>
//         </div>
//         {!readOnly && (
//           <button
...
//         ) : (
//           <>
//             {/* Column labels — only on first gap in section */}
//             <div className="grid gap-2.5 px-3 pb-1 text-[9px] font-bold uppercase tracking-wide text-forest-400"
//               style={{ gridTemplateColumns:'2.5fr 1.5fr 1fr 1fr 1.5fr 2fr 28px' }}>
...
// // ─── Dimension section — shows ALL gaps in the dimension ───────
// function DimSection({ dim, gapsInDim, cdpRows, allIndices, onUpdate, onRemove, onAddAction, t, readOnly }: {
//   dim: string
//   gapsInDim: string[]
//   cdpRows: CdpRow[]
//   allIndices: number[]
...
//   const totalActions = cdpRows.length

//   return (
//     <div className="mb-5 rounded-2xl overflow-hidden border border-sand-300">
//       {/* Dimension header */}
//       <div className="flex items-center justify-between px-4 py-2.5"
//         style={{ background:'#1b4332' }}>
//         <div className="flex items-center gap-3">
//           <span className="text-[12px] font-bold text-white tracking-wide">{shortDim}</span>
//           <span className="text-[10px] opacity-60 text-green-200">
//             {gapsInDim.length} gap{gapsInDim.length!==1?'s':''} · {totalActions} action{totalActions!==1?'s':''}
//           </span>
//         </div>
//       </div>

//       {/* All gaps in this dimension — stacked */}
//       <div className="p-4">
//         {gapsInDim.map(gap => (
//           <GapSubsection
//             key={gap}
...
//   // Prioritized gaps from PriorityPage
//   const gaps = gapItems(assessment)

//   // Build dim → gap labels map (from prioritization)
//   const dimGaps: Record<string, string[]> = {}
//   gaps.forEach(g => {
//     if (!dimGaps[g.dim]) dimGaps[g.dim] = []
//     if (!dimGaps[g.dim].includes(g.label)) dimGaps[g.dim].push(g.label)
//   })

//   // Dimensions that have at least one prioritized gap
//   const activeDims = DIMENSIONS.filter(d => dimGaps[d]?.length > 0)

//   // CDP rows grouped by dimension (derived from their capacityGap)
//   function dimForGap(gapLabel: string): string {
//     return gaps.find(g => g.label === gapLabel)?.dim ?? ''
//   }

//   function rowsForDim(dim: string): { rows: CdpRow[]; indices: number[] } {
//     const rows: CdpRow[]   = []
//     const indices: number[] = []
//     cdpRows.forEach((r, i) => {
//       if (dimForGap(r.capacityGap) === dim || (!r.capacityGap && dim === activeDims[0])) {
//         rows.push(r); indices.push(i)
//       }
//     })
//     return { rows, indices }
//   }

//   function handleAddAction(dim: string, gap: string) {
//     addCdpRow(gap)
//   }
...
//   const totalGaps    = Object.values(dimGaps).flat().length
...
//             <div className="text-[22px] font-light" style={{ fontFamily:'var(--font-mono)', color:'#2d6a4f' }}>
//               {totalActions}
//             </div>
//             <div className="text-[10.5px] text-forest-400 uppercase tracking-wide">
//               actions planned
//             </div>
//           </div>
//         )}
...
//           <div className="text-[13px] text-forest-400 mb-1">No prioritized gaps found.</div>
//           <div className="text-[12px] text-forest-300">
//             Complete the Gap Analysis and Prioritization pages first, then return here to plan capacity development actions.
//           </div>
//         </div>
//       ) : (
//         activeDims.map(dim => {
//           const { rows, indices } = rowsForDim(dim)
//           return (
//             <DimSection
//               key={dim}
//               dim={dim}
//               gapsInDim={dimGaps[dim]}
//               cdpRows={rows}
//               allIndices={indices}
...
const ActionRow = memo(function ActionRow({ idx, initialRow, onUpdate, onRemove, t, readOnly }: {
  ...
  return (
    <div className="grid gap-2.5 p-3 rounded-xl bg-sand-50 border border-sand-300 mb-2 group relative"
      ...
      {ta('institution',   t.common.institution)}
      <div>
        <div className="text-[9px] font-bold uppercase tracking-wide text-forest-400 mb-1">Timeline</div>
        <select className="ti w-full" style={{ appearance:'none', cursor: readOnly ? 'default' : 'pointer' }}
          value={fromCanonical(TIMELINE_CANONICAL, t.cdp.timelines, vals.timeline)}
          disabled={readOnly}
          onChange={e => {
      ...
      <div className="px-3 pt-2 pb-3">
        {paired.length === 0 ? (
          ...
          <>
            <div className="grid gap-2.5 px-3 pb-1 text-[9px] font-bold uppercase tracking-wide text-forest-400"
              style={{ gridTemplateColumns:'2.5fr 1.5fr 1fr 1fr 1.5fr 2fr 28px' }}>
              <span>{t.common.action}</span><span>{t.common.institution}</span>
              <span>Timeline</span><span>{t.common.budget}</span>
              <span>{t.common.indicator}</span><span>{t.common.collaboration}</span><span/>
            </div>
            {paired.map(({ row, globalIdx }) => (
