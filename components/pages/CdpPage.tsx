...
//   // Sync when store reloads from DB after save
//   useEffect(() => {
//     setVals({
//       capacityGap: initialRow.capacityGap, action: initialRow.action,
//       institution: initialRow.institution, timeline: initialRow.timeline,
//       budget: initialRow.budget, indicator: initialRow.indicator, collaboration: initialRow.collaboration,
//     })
//   }, [initialRow.capacityGap, initialRow.action, initialRow.institution,
//       initialRow.timeline, initialRow.budget, initialRow.indicator, initialRow.collaboration])
...
import { type CdpRow } from '@/lib/constants'
import { getT } from '@/lib/i18n'
import { TIMELINE_CANONICAL, toCanonical, fromCanonical } from '@/lib/utils'
import { SectionActions } from '@/components/ui'
  ...
  return (
    <div className="grid gap-2 items-start p-3.5 mb-2 bg-white rounded-xl border border-sand-300 hover:shadow-sm transition-shadow"
      ...
      {COLS.map(c => (
        <div key={c.k}>
          {c.sel ? (
            <select className="ti w-full" style={{appearance:'none',cursor:'pointer'}}
              value={fromCanonical(TIMELINE_CANONICAL, t.cdp.timelines, vals[c.k])}
              onChange={e => {
                const canonical = toCanonical(t.cdp.timelines, e.target.value, TIMELINE_CANONICAL)
                setVals(p => ({...p, [c.k]: canonical}))
                onUpdate(idx, c.k, canonical)
              }}>
              {t.cdp.timelines.map(tl => <option key={tl} value={tl}>{tl||t.common.selectDots}</option>)}
            </select>
          ) : (
