        ...
        <div className="grid grid-cols-2 gap-3.5 mb-3.5">
          ...
          <div className="flex flex-col gap-1.5">
            ...
            <select className="form-input"
              value={fromCanonical(INST_TYPES_CANONICAL, p.types, profile.type)}
              onChange={e => updateProfile('type', toCanonical(p.types, e.target.value, INST_TYPES_CANONICAL))}>
              <option value="">{t.common.selectDots}</option>
              {p.types.map(tp => <option key={tp}>{tp}</option>)}
            </select>
          </div>
          <div className="flex flex-col gap-1.5">
            <label className="text-[11px] font-bold text-forest-400 uppercase tracking-wide">{p.level}</label>
            <select className="form-input"
              value={fromCanonical(INST_LEVELS_CANONICAL, p.levels, profile.level)}
              onChange={e => updateProfile('level', toCanonical(p.levels, e.target.value, INST_LEVELS_CANONICAL))}>
              <option value="">{t.common.selectDots}</option>
              {p.levels.map(lv => <option key={lv}>{lv}</option>)}
            </select>
          </div>
