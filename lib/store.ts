...
interface Store {
  ...
  // CDP
  addCdpRow:    (capacityGap?: string, source?: 'core' | 'target', dimension?: string, tNum?: number) => void
...
export const useStore = create<Store>()(
  persist(
    (set, get) => ({
      ...
      addCdpRow: (capacityGap = '', source?: 'core' | 'target', dimension?: string, tNum?: number) => set(s => ({
        assessment: { ...s.assessment, cdpRows: [...s.assessment.cdpRows, { capacityGap, action:'', institution:'', timeline:'', budget:'', indicator:'', collaboration:'', source, dimension, tNum }] }
      })),
