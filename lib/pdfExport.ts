...
export async function exportNationalPDF(
  ...
  // Column layout — institution name col + overall + 8 dim cols
  const instColW = 65
  const overallW = 16
  ...
  national.institutions.forEach((inst, rowIdx) => {
    ...
    // Institution name — truncate if needed
    doc.setFontSize(7).setTextColor(27,67,50).setFont('helvetica', inst.overallScore !== null ? 'bold' : 'normal')
    const maxChars = 42
    const name = inst.institution.name.length > maxChars ? inst.institution.name.slice(0, maxChars-1)+'…' : inst.institution.name
    doc.text(name, LML+2, y+4.5)

    if (inst.overallScore !== null) {
      // Overall score
      doc.setFontSize(7.5).setTextColor(...hex2rgb(scoreColor(inst.overallScore))).setFont('helvetica','bold')
      doc.text(inst.overallScore.toFixed(2), LML + instColW + overallW/2, y+4.5, { align:'center' })
      ...
      DIMENSIONS.forEach((d, i) => {
        ...
        doc.text(sv !== null ? sv.toFixed(1) : '—', cx, y+4.5, { align:'center' })
      })
    } else {
      doc.setFontSize(7).setTextColor(160,160,160).setFont('helvetica','italic')
      doc.text('No assessment data', LML + instColW + 5, y+4.5)
    }
    y += rowH
  })
