...
//     coreRows: CORE_QUESTIONS.map(() => ({ score:null, evidence:'', gap:'', capacityType:'', priority:'', suggestedSupport:'' })),
//     targetRows: {},
...
//   const scores = indicators.map((_,i) => a.targetRows[`t${num}_${i}`]?.score).filter((s): s is number => s !== null)
//   return scores.length ? scores.reduce((x,y) => x+y, 0) / scores.length : null
// }
...
// export function interpret(v: number | null): string {
//   if (v === null) return 'Not assessed'
//   if (v < 1) return 'Critical – no capacity'; if (v < 2) return 'Very limited'
