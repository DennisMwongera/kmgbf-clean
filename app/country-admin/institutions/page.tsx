...
const EMPTY_FORM = { name:'', type:'', level:'' }
...
export default function CountryAdminInstitutionsPage() {
  ...
  async function load(cid: string) {
    try {
      const { data: insts } = await supabase
        .from('institutions').select('id, name, type, level').eq('country_id', cid).order('name')
      ...
      userCounts?.forEach(r => { if (r.institution_id) userMap[r.institution_id] = (userMap[r.institution_id] ?? 0) + 1 })
  ...
  function openEdit(inst: Institution) { setEditTarget(inst); setForm({ name: inst.name, type: inst.type ?? '', level: inst.level ?? '' }); setError(''); setShowForm(true) }
  ...
  async function handleSave(e: React.FormEvent) {
    ...
    if (editTarget) {
      ...
      const { error } = await supabase.from('institutions')
        .update({ name: form.name.trim(), type: form.type || null, level: form.level || null })
        .eq('id', editTarget.id)
  ...
  async function handleDelete(inst: Institution) {
    if (inst.answered_count > 0 || inst.user_count > 0) { setConfirmDel(inst); return }
    doDelete(inst.id)
  }
  ...
  const modalTitle = editTarget ? 'Edit Institution' : 'Add Institution'

  return (
    <div className="fade-in">
      <div className="flex items-start justify-between mb-6 flex-wrap gap-4">
        <div>
          <h2 style={{ fontFamily:'var(--font-display)', fontSize:26, fontWeight:700, color:'#0f2d1c' }}>Institutions</h2>
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-2xl border border-sand-300 overflow-hidden">
          <table className="rt w-full">
            <thead>
              <tr><th>Institution</th><th>Score</th><th>Progress</th><th>Status</th><th>Users</th><th>Updated</th><th></th></tr>
            </thead>
            <tbody>
              {institutions.map(i => (
                <tr key={i.id}>
                  <td>
                    ...
                    {i.level && <div className="text-[10px] text-forest-300">{i.level}</div>}
                  </td>
                  <td>
                  ...
                  <td>
                    <div className="flex items-center gap-1">
                      ...
                      <button onClick={() => openEdit(i)}
                        ...
                        style={{ color:'#3b82f6' }} title="Edit institution">
                        <Pencil size={12}/>
                      </button>
                      <button onClick={() => handleDelete(i)} disabled={deleting === i.id}
                        ...
                        style={{ color:'#dc2626' }} title="Delete institution">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4"
          ...
          <div className="bg-white rounded-2xl p-7 w-full max-w-md" style={{ boxShadow:'0 24px 60px rgba(0,0,0,.25)' }}>
            <div className="flex items-center justify-between mb-5">
              <h3 style={{ fontFamily:'var(--font-display)', fontSize:20, fontWeight:700, color:'#0f2d1c' }}>{modalTitle}</h3>
