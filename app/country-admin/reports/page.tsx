'use client'
// Country Admin Reports — scoped to the country admin's country
import { useEffect, useRef, useState } from 'react'
import { supabase } from '@/lib/supabase/client'
import { loadAllInstitutionReports, buildNationalReport, loadNationalCdpRows, loadArchivedCdpRows, archiveCdpRow, restoreCdpRow, archiveCdpRowByKey, restoreCdpRowByKey, archiveGapRows, restoreGapRows, type InstitutionReport, type NationalReport, type NationalCdpRow } from '@/lib/supabase/adminApi'
...
export function NationalReportView({ countryId: propCountryId, countryName: propCountryName }: { countryId?: string; countryName?: string }) {
  ...
  async function handleArchive(row: NationalCdpRow) {
    ...
    if (ok) {
      ...
      if (showArchived) {
        const archived = await loadArchivedCdpRows([...selectedIds], countryId ?? undefined)
        ...
        <div>
          <SectionHeader icon={Table2} title="National Capacity Development Plans">
            <ExportMenu mini options={[
              { label:'CSV — Core',    icon:'📋', action:() => downloadCSV([
                  ...
                  ...cdpRows
                    .filter(r => r.source === 'core' && !/^T\d+:/.test(r.capacity_gap ?? ''))
                    .map(r=>[r.institution_name,r.dimension??'Unclassified',r.capacity_gap??'',r.action??'',r.institution??'',r.timeline??'',r.budget_usd??'',r.indicator??'',r.collaboration??'',r.assessment_status??''])
                ], `National_Core_CDP_${date}`) },
              { label:'CSV — Targets', icon:'📋', action:() => downloadCSV([
                  ['Institution','Target #','Target','Capacity Gap','Action','Responsible','Timeline','Budget (USD)','Indicator','Collaboration','Status'],
                  ...cdpRows
                    .filter(r => r.source === 'target' || /^T\d+:/.test(r.capacity_gap ?? ''))
                    .sort((a,b) => (a.target_num??0) - (b.target_num??0))
                    .map(r=>[
                      r.institution_name,
                      r.target_num ? `T${r.target_num}` : '—',
          ...
          {cdpLoading ? (
            ...
            targetRows.forEach(r => {
              // tNum=null means indicator-level gap saved without T-prefix.
              // Try to extract from capacity_gap pattern, else group under 0.
              let n = r.target_num ?? 0
              // Try harder: extract from gap text like "T7: Pollution — ..."
              if (!n && r.capacity_gap) {
                const m = r.capacity_gap.match(/^T(\d+):/)
                if (m) n = parseInt(m[1])
              }
              const title = n
                ? (r.target_title ?? `T${n}`)
                : 'Unassigned Target Gaps'
              if (!byTarget[n]) byTarget[n] = { title, insts:{} }
              ...
              <>
                ...
                <div className="grid grid-cols-4 gap-3 mb-6">
                  {[
                    ...
                    { label:'Target Actions', value: targetRows.length,                         color:'#1d4ed8' },
                  ...
                  <div className="mb-6">
                    <div className="flex items-center gap-3 mb-5">
                      ...
                      <span className="chip text-[10px]" style={{ background:'#dbeafe', color:'#1d4ed8' }}>
                        {targetRows.length} row{targetRows.length!==1?'s':''} across {Object.keys(byTarget).length} target{Object.keys(byTarget).length!==1?'s':''}
                      </span>
                    </div>
