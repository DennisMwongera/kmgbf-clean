...
import { Building2, Users, ArrowLeft, Loader2, ChevronRight, FileCheck, BarChart2, Globe, Target, Table2 } from 'lucide-react'
import Link from 'next/link'
import { NationalReportView } from '@/app/country-admin/reports/page'

interface Props { params: { id: string } }
interface Institution {
  id: string; name: string; type: string | null; level: string | null
  status: string | null; answered_count: number; overall_score: number | null; updated_at: string | null
}
type Tab = 'institutions' | 'users' | 'reports'
...
export default function SuperAdminCountryDetailPage({ params }: Props) {
  ...
  useEffect(() => {
    async function load() {
      const { data: c } = await supabase.from('countries').select('*').eq('id', params.id).single()
      setCountry(c)

      const { data: insts } = await supabase.from('institutions')
        .select('id, name, type, level').eq('country_id', params.id).order('name')
      ...
      assessments?.forEach(a => { if (!latestByInst[a.institution_id]) latestByInst[a.institution_id] = a })
      ...
      setInstitutions(insts.map(inst => {
        ...
        return { ...inst, status: a?.status??null, updated_at: a?.updated_at??null, answered_count: aid?(countMap[aid]??0):0, overall_score: scores.length?scores.reduce((x,y)=>x+y,0)/scores.length:null }
      }))

      const { data: users } = await supabase.from('user_profiles')
        .select('id, full_name, email, role, institution_id')
        .eq('country_id', params.id).order('role')
      const iMap: Record<string,string> = {}
      insts.forEach(i => { iMap[i.id] = i.name })
      setUsers((users ?? []).map(u => ({ ...u, institution_name: u.institution_id ? (iMap[u.institution_id]??null) : null })))
      setLoading(false)
    }
    load()
  }, [params.id])
  ...
  const TABS = [
    { id:'institutions' as Tab, label:`Institutions (${institutions.length})`, Icon: Building2 },
    { id:'users'        as Tab, label:`Users (${users.length})`,               Icon: Users     },
    { id:'reports'      as Tab, label:'National Reports',                       Icon: BarChart2 },
  ]
  ...
  return (
    <div className="fade-in">
      ...
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          <h2 style={{ fontFamily:'var(--font-display)', fontSize:24, fontWeight:700, color:'#0f2d1c' }}>{country?.name}</h2>
      ...
      <div className="grid grid-cols-3 gap-4 mb-6">
        {[
          ...
          <div key={label} className="bg-white rounded-xl px-5 py-4 border border-sand-300" style={{ boxShadow:'0 2px 8px rgba(15,45,28,.06)' }}>
            <div className="flex items-center gap-1.5 text-[10px] font-bold tracking-[1.5px] uppercase text-forest-400 mb-1"><Icon size={10}/> {label}</div>
      ...
      {activeTab === 'institutions' && (
        <div className="bg-white rounded-2xl border border-sand-300 overflow-hidden">
          <table className="rt w-full">
            <thead><tr><th>Institution</th><th>Score</th><th>Progress</th><th>Status</th><th>Updated</th><th></th></tr></thead>
            <tbody>
              {institutions.length === 0
                ...
                : institutions.map(i => (
                  <tr key={i.id}>
                    ...
                    <td>{i.overall_score !== null ? <span className="font-bold" style={{ fontFamily:'var(--font-mono)', color:scoreColor(i.overall_score) }}>{i.overall_score.toFixed(2)}</span> : <span className="text-forest-300">—</span>}</td>
                    <td>
                    ...
                    <td>{i.status ? <span className="chip text-[10px]" style={{ background:i.status==='approved'?'#d8f3dc':i.status==='submitted'?'#dbeafe':i.status==='in_review'?'#ede9fe':'#fef3c7', color:i.status==='approved'?'#1b4332':i.status==='submitted'?'#1d4ed8':i.status==='in_review'?'#6d28d9':'#d97706' }}>{i.status.replace('_',' ')}</span> : <span className="text-[10px] text-forest-300">Not started</span>}</td>
                    <td className="text-[11px] text-forest-400">{i.updated_at ? new Date(i.updated_at).toLocaleDateString() : '—'}</td>
                    <td>
      ...
      {/* ── National Reports tab — full detail ── */}
      {activeTab === 'reports' && (
        <NationalReportView countryId={params.id} countryName={country?.name ?? ''} />
      )}
